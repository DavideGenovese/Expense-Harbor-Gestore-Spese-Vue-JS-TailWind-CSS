...
<template>
  ...
  <div class="flex justify-end items-center sm:px-5 sm:py-2">
    <button
      @click="createNew"
      type="button"
  ...
  <div class="mx-4 overflow-x-auto shadow-md sm:rounded-lg">
    ...
    >
      <thead
        class="text-sm uppercase bg-gray-700 text-gray-400"
      >
        <tr>
      ...
      <tbody v-if="list" v-for="item in list" :key="item.id">
        <tr class=" border-b bg-gray-800 border-gray-700">
          ...
          <td class="px-6 py-4 text-[17px] text-white">{{ item.descrizione }}</td>
          <td class="px-6 py-4 text-[17px] text-white">€ {{ parseFloat(item.importo).toFixed(2) }}</td>
